...
from lib2.meta_model import LeNet, VNet, LeNet_D 
...
parser.add_argument("--val_num", default=5000, type=int, help="val number")
...
val_loader_in_train = DataLoader(l_train_dataset, args.meta_val_batch, drop_last=True,
                                 sampler=RandomSampler(len(l_train_dataset), shared_cfg["iteration"][args.dataset] * args.meta_val_batch))
...
for p in range(10):
    ...
    opt_w = optim.Adam(vnet.params(), lr=args.meta_lr)
    ood_result = []
    in_result = []
    ...
    for l_data, u_data, v_data in zip(l_loader, u_loader, val_loader_in_train):
        ...
        with higher.innerloop_ctx(model, optimizer) as (meta_model, diffopt):
            ...
            cost_v = torch.reshape(ssl_loss_meta, (len(ssl_loss_meta), 1))
            v_lambda = vnet(cost_v.data)
            ssl_loss_meta = ssl_loss_meta * v_lambda
        ...
        ssl_loss = ssl_obj(u_input, outputs_u.detach(), model)
        cost_v = torch.reshape(ssl_loss, (len(ssl_loss), 1))
        with torch.no_grad():
            w_new = vnet(cost_v)
        ssl_loss = (ssl_loss * w_new).mean()
        ...
        w_in = w_new[(idx < iidx)].data.cpu().numpy().mean()
        w_ood = w_new[(idx >= iidx)].data.cpu().numpy().mean()
        ...
        if iteration == 1 or (iteration % 100) == 0:
            ...
            print("iteration [{}/{}] cls loss : {:.6e}, SSL loss : {:.6e}, w_max : {:.2f}, w_mean : {:.2f}, time : {:.3f} iter/sec, rest : {:.3f} min, lr : {}".format(
    ...
    writer.close()
    # np.save('output/weight/{}_MWN_mix_MNIST_ood_w_{}_{}'.format(args.alg, args.ood_ratio, p), ood_result)
    # np.save('output/weight/{}_MWN_mix_MNIST_in_w_{}_{}'.format(args.alg, args.ood_ratio, p), in_result)
